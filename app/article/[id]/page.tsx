...
function SourceInitial({ name }: { name: string }) {
  return (
    <div className="w-6 h-6 rounded-full bg-stone-200 flex items-center justify-center flex-shrink-0">
      <span className="text-body-sm font-semibold text-stone-500 leading-none">{name.charAt(0)}</span>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-body-md text-gray-500 hover:text-gray-700 mb-6"
        >
        ...
        <div className="mb-4">
          <Link
            ...
            className="inline-block px-3 py-1 rounded-full text-body-sm font-medium bg-[#D3BDB0]/30 text-stone-800 hover:bg-[#D3BDB0]/50 transition-colors"
          >
            {article.primaryArea}
          </Link>
          {article.lowConfidenceTag && (
            <span className="ml-2 inline-block px-2 py-0.5 rounded-full text-body-sm bg-yellow-100 text-yellow-700">
              auto-tagged
            </span>
          )}
        </div>
        ...
        {/* Headline */}
        <h1 className="font-serif text-display-3 md:text-display-2 font-semibold text-gray-900 mb-5">
          {article.title}
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-body-md font-medium text-gray-700">{article.source.name}</span>
          </div>
          <TimeAgo date={article.publishedAt} />
          <span className="px-2 py-0.5 rounded text-body-sm font-medium bg-gray-100 text-gray-600 uppercase">
            {article.lang}
          </span>
          {article.author && (
            <span className="text-body-md text-gray-400">by {article.author}</span>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {summaryParagraphs.length > 0 && (
              <div>
                <h2 className="text-body-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">
                  Summary
                </h2>
                <div className="space-y-3">
                  {summaryParagraphs.map((p, i) => (
                    <p key={i} className="text-body-lg text-gray-700">
                      {p}
                    </p>
                  ))}
                </div>
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors text-body-md font-medium"
            >
            ...
            {article.cluster ? (
              <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
                ...
                <div className="p-4 space-y-3">
                  <h3 className="text-body-sm font-semibold text-gray-500 uppercase tracking-wide">
                    Coverage
                  </h3>

                  <div className="space-y-1">
                    <span className="font-serif text-display-2 font-bold text-[#2B7878] leading-none">
                  ...
                  {article.cluster.hasIndependentVoice && (
                    <p className="text-body-md font-medium text-[#8D165F]">
                  ...
                  {article.cluster.totalSourceCount > 1 && (
                    <div className="flex flex-wrap gap-1.5">
                      {typeCounts.wire > 0 && (
                        <span className="px-2 py-0.5 rounded-full text-body-sm font-medium bg-stone-100 text-stone-600">
                  ...
                  {corroborationScore !== null && article.cluster.totalSourceCount > 1 && (
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-body-sm text-gray-400">Corroboration</span>
                        <span className="text-body-sm font-medium text-gray-600">{corroborationScore}%</span>
                      </div>
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2 mb-0.5">
                              ...
                              <span className="text-body-sm text-gray-400 flex-shrink-0">
                            ...
                            <h4 className="font-serif text-headline-sm font-medium text-gray-900 line-clamp-2 mb-0.5">
                              {ca.title}
                            </h4>
                            {ca.excerpt && (
                              <p className="text-body-sm text-gray-400 line-clamp-1">
                                {ca.excerpt}
                              </p>
                            )}
                          </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h3 className="text-body-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">
                  Coverage
                </h3>
                <p className="text-body-md text-gray-400">Not clustered yet.</p>
              </div>
            )}
          </div>
        </div>
